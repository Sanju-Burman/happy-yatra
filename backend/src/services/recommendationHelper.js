...
// Store pending generation promises to prevent duplicate concurrent API calls
...
async function generateAndCacheRecommendations(userId, survey) {
    ...
    const cacheKey = `${userId}-${surveyHash}`;

    // Deduplicate concurrent requests
    if (pendingGenerations.has(cacheKey)) {
        console.log(`[Cache] Joining existing pending generation for ${cacheKey}`);
    ...
    const generationPromise = (async () => {
        ...
        console.log(`[Gemini] Starting generation for ${cacheKey}...`);
