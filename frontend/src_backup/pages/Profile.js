      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-heading text-4xl font-bold text-secondary mb-2 tracking-tight">
                {profile?.user?.name}
              </h1>
              <p className="text-gray-600 mb-4">{profile?.user?.email}</p>
              
              <div className="flex gap-6 mt-6">
                <div className="bg-[#F2EFE9] px-6 py-3 rounded-lg">
                  <div className="flex items-center gap-2 mb-1">
                    <Heart className="w-5 h-5 text-primary" strokeWidth={1.5} />
                    <span className="text-sm text-gray-600">Saved</span>
                  </div>
                  <p className="text-2xl font-bold text-secondary">{profile?.saved_destinations_count || 0}</p>
                </div>
                ...
                {profile?.preferences && (
                  <div className="bg-[#F2EFE9] px-6 py-3 rounded-lg">
                    ...
                    <p className="text-lg font-semibold text-secondary capitalize">{profile.preferences.travel_style}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {profile?.preferences && (
            <div className="mt-8 pt-8 border-t border-gray-200">
              <h2 className="font-heading text-2xl font-bold text-secondary mb-6">Your Preferences</h2>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profile.preferences.interests.map((interest, idx) => (
                      <span key={idx} className="bg-[#F2EFE9] px-4 py-2 rounded-full text-sm font-medium text-secondary">
                        {interest}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <span className="bg-[#F2EFE9] px-4 py-2 rounded-full text-sm font-medium text-secondary capitalize">
                    {profile.preferences.budget}
                  </span>
                </div>

                <div>
                  <h3 className="font-medium text-gray-700 mb-3">Regions Visited</h3>
                  <div className="flex flex-wrap gap-2">
                    {profile.preferences.past_travels.map((region, idx) => (
                      <span key={idx} className="bg-[#F2EFE9] px-4 py-2 rounded-full text-sm font-medium text-secondary flex items-center gap-1">
        ...
        >
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="font-heading text-3xl font-bold text-secondary">Saved Destinations</h2>
          </div>

          {savedDestinations.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              <Heart className="w-16 h-16 text-gray-300 mx-auto mb-4" strokeWidth={1.5} />
              <p className="text-gray-600 mb-4">You haven't saved any destinations yet</p>
              <a
                href="/recommendations"
                className="text-primary hover:text-[#A04B32] font-medium"
              >
            ...
            <div data-testid="profile-saved-destinations" className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {savedDestinations.map((destination, idx) => (
                <motion.div
                  key={destination.id}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
